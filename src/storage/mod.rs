...
impl AuthenticationStorage {
    ...
    pub fn set_api_key(api_key: &String) {
        ...
        storage.store(AuthenticationStorage::get_api_key_storage_key(), Json(api_key));
    }
    ...
    pub fn set_device_code(device_code: &String) {
        let mut storage = StorageService::new(Area::Local).unwrap();
        storage.store(AuthenticationStorage::get_device_code_storage_key(), Json(device_code));
    }

    pub fn set_username(username: &String) {
        let mut storage = StorageService::new(Area::Session).unwrap();
        storage.store(AuthenticationStorage::get_username_storage_key(), Json(username));
    }

    pub fn set_password(password: &String) {
        let mut storage = StorageService::new(Area::Session).unwrap();
        storage.store(AuthenticationStorage::get_password_storage_key(), Json(password));
    }
