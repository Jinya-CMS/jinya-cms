...
class Theme extends LoggingEntity
{
    #[Id]
    ...
    public static function getActiveTheme(): ?self
    {
        ...
        $logger->debug('Find active theme');
    ...
    {
        $logger = Logger::getLogger();
        $logger->debug('Find theme by name', ['name' => $name]);
        $query = self::getQueryBuilder()
            ->newSelect()
    ...
    {
        $this->logger->debug('Make theme active', ['themeId' => $this->id, 'name' => $this->name]);
        $this->logger->debug('Delete current entry in configuration table');
        $query = self::getQueryBuilder()
            ->newDelete()
            ->from('configuration');
        ...
        $this->logger->debug('Recreate entry in configuration table');
        $query = self::getQueryBuilder()
            ->newInsert()
            ->into('configuration')
