  ...
  class p {
    ...
    _createSortedPathsRules() {
      this.sortedPathsRules = [], u.Utilities.forEachProperty(this.options.paths, (r, c) => {
        Array.isArray(c) ? this.sortedPathsRules.push({
          from: r,
          to: c,
    ...
    moduleIdToPaths(r) {
      ...
      if (!u.Utilities.endsWith(c, '.js') && !u.Utilities.isAbsolutePath(c)) {
        ...
        !u.Utilities.endsWith(c, '.js') && !u.Utilities.containsQueryString(c) && (c = c + '.js'), a = [c];
      }
...
(function(u) {
    ...
    load(e, i, s, n) {
      if (!this._scriptLoader) {
        if (this._env.isWebWorker) {
          ...
          const { preferScriptTags: d } = e.getConfig()
            .getOptionsLiteral();
          d ? this._scriptLoader = new m : this._scriptLoader = new r(this._env);
        } else {
          this._env.isNode ? this._scriptLoader = new r(this._env) : this._scriptLoader = new m;
        }
      }
      let l = {
        callback: s,
  ...
  class m {
    ...
    load(e, i, s, n) {
      if (/^node\|/.test(i)) {
        ...
        const { trustedTypesPolicy: d } = e.getConfig()
          .getOptionsLiteral();
        d && (i = d.createScriptURL(i)), l.setAttribute('src', i);
        const { cspNonce: o } = e.getConfig()
          .getOptionsLiteral();
  ...
  function p(t) {
    const { trustedTypesPolicy: e } = t.getConfig()
      .getOptionsLiteral();
    try {
  ...
  class h {
    ...
    load(e, i, s, n) {
      if (/^node\|/.test(i)) {
        ...
        const { trustedTypesPolicy: l } = e.getConfig()
          .getOptionsLiteral();
...
//# sourceURL=${i}`, (l ? self.eval(l.createScript('', o)) : new Function(o)).call(self), s();
  ...
  class r {
    ...
    _initNodeRequire(e, i) {
      const { nodeCachedData: s } = i.getConfig()
        .getOptionsLiteral();
    ...
    _handleCachedData(e, i, s, n, l) {
      e.cachedDataRejected ? this._fs.unlink(s, d => {
        l.getRecorder()
          .record(62, s), this._createAndWriteCachedData(e, i, s, l), d && l.getConfig()
          .onError(d);
      }) : n && this._createAndWriteCachedData(e, i, s, l);
    }

    _createAndWriteCachedData(e, i, s, n) {
      let l = Math.ceil(n.getConfig()
      ...
      const f = () => {
        setTimeout(() => {
          _ || (_ = this._crypto.createHash('md5')
            .update(i, 'utf8')
            .digest());
    ...
    _verifyCachedData(e, i, s, n, l) {
      n && (e.cachedDataRejected || setTimeout(() => {
        const d = this._crypto.createHash('md5')
          .update(i, 'utf8')
          .digest();
        n.equals(d) || (l.getConfig()
          .onError(new Error(`FAILED TO VERIFY CACHED DATA, deleting stale '${s}' now, but a RESTART IS REQUIRED`)), this._fs.unlink(s, o => {
          o && l.getConfig()
            .onError(o);
        }));
...
(function(u) {
  ...
  class m {
    ...
    static _safeInvokeFunction(t, e) {
      try {
        return {
          returnedValue: t.apply(u.global, e),
          producedError: null,
        };
      } catch (i) {
        return {
      ...
      if (e) {
        for (let i = 0, s = e.length; i < s; i++) {
          ...
          if (this._hasDependencyPath(n.id, t.id)) {
            ...
            d.reverse(), d.push(n.id), console.warn(d.map(o => this._moduleIdProvider.getStrModuleId(o))
              .join(` => 
`)), t.unresolvedDependenciesCount--;
            continue;
          }
