...
a theme has, the SCSS, JS files to use and a list of assets that should get copied into the public folder.
...
Jinya analyzes the specified variables file and then displays the variables in the designer. The array key used is
styles.
...
Every configuration group contains of a key and an array containing the configuration keys and the default values. As
...
Every group needs to contain the group name, specified in the default configuration. Since the group names are rather
